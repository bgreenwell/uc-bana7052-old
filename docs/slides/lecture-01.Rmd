...
---
class: clear, center, middle
background-image: url("images/curve-fitting.png")
background-size: contain
...
background-image: url("images/read-the-book.jpg")
background-size: contain
...
```{r statistical-relationships-04, echo=FALSE, out.width="90%"}
# Display plots in a grid
gridExtra::grid.arrange(p1, p2, p3, p4, ncol = 2)
```
...
```{r pearson-correlation-01, echo=FALSE, fig.width=7, fig.height=5, out.width="90%"}
set.seed(101)
x <- seq(from = -2, to = 2, length = 100)
d <- data.frame(
  y1 = 1 + x + rnorm(length(x), sd = 1/3),
  y2 = 1 - x + rnorm(length(x), sd = 1/3),
  y3 = 1 + x^2 + rnorm(length(x), sd = 1/3),
  y4 = 1 + rnorm(length(x), sd = 1/3)
)
p1 <- ggplot(d, aes(x = x, y = y1)) +
  geom_point(alpha = 2/3) + 
...
p4 <- "images/correlation-coefficient.png" %>%
  png::readPNG() %>%
  grid::rasterGrob(interpolate = TRUE, width = 0.6)
p4 <- ggplot() +
...
```{r pvalue-simulation}
# See ?set.seed, ?replicate, and ?cor.test for details
set.seed(1926)
pvals <- replicate(1000, {
  cor.test(x = rnorm(10000), y = rnorm(10000))$p.value
})
mean(pvals <= 0.05)
```
...
head(crystal, n = 5)  # print first five observations
```
...
```{r your-turn-01-03, fig.width=6, fig.asp=0.618, out.width="70%"}
# Scatterplot using lattice
# library(lattice)  # or use :: like below
lattice::xyplot(weight ~ time, data = crystal, pch = 19, cex = 1.3)
```
...
---
class: clear, middle, center
background-image: url("images/slr-conditional-distribution.png")
background-size: contain
...
background-image: url("images/calculus-joke.jpg")
background-size: contain
