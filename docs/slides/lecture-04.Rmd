...
* PRESS statistic: $PRESS = \sum_{i=1}^n d_i^2$ (.red[useful for mdoel selection])
...
* Misspecified mean structure: transformations and nonparametric algorithms (e.g., MARS or a decision tree), etc.
...
Return to the Boston housing example from the previous lecture. Fit an MLR model using `cmed` as the response and `lstat` and `rm` as the predictors. Check the adquecy of the fitted model using the methods discussed in this lecture. Does the model appear adequate? What assumptions appear to be violated? Are there outlying and/or influential values?
