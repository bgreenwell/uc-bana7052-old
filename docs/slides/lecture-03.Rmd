...
This is one reason why we dumme encode categorical variables
...
Statsitcial inference in the MLR model is not that different from inference in SLR (only a slight change in the formulas)
...
Fit an MLR to the Boston housing data using all of the available predictors. Is the regression significant? Use the general *F*-test to determine whether the model with only the predcitors `lstat` and `rm` is adequate.
...
The absolute value of the $t$-statistics is a reasonable indicator for the relative importance of each predcitor in a linear model
...
* It is never a good idea to make predicitons outside the region covered by the training data
    ...
    - $X_{new}$ is concidered "outlying" if $h_{new} = \boldsymbol{X}_{new}^\top\left(\boldsymbol{X}^\top\boldsymbol{X}\right)^{-1}\boldsymbol{X}$ lies outside of the range of the diagonal entries of $\boldsymbol{H}$
...
* A *k*-th order model in a signle predictor $$Y = \beta_0 + \sum_{j=1}^k\beta_j X^j + \epsilon$$ 
